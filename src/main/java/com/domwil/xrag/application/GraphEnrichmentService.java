...
public class GraphEnrichmentService {
    ...
    /**
     * Extrait les sujets du document, crée les nœuds TOPIC (+ alias) reliés aux nœuds
     * NON-topic du document (page/issue/class) — et non plus au nœud PROJECT global :
     * l'étoile autour du hub rendait le boost non discriminant et le sous-graphe vide
     * (revue 2026-07, H2). Renvoie les ids à rattacher aux chunks ; vide si aucun sujet.
     */
    private LinkedHashSet<String> topicNodesFor(UnattachedDocument document) {
        ...
        var topicNodes = new java.util.ArrayList<GraphNode>();
        ...
        for (String topic : topics) {
            ...
            topicNodes.add(GraphNode.of(topicId, GraphNode.Types.TOPIC, topic));
            for (String documentNodeId : document.existingNodeIds()) {
                edges.add(GraphEdge.of(topicId, documentNodeId, GraphEdge.Types.REFERENCES));
            }
        ...
        // ExtractionResult construit directement : seuls les nœuds TOPIC sont upsertés — les
        // cibles des arêtes (page/issue/class) existent déjà en base et ne doivent JAMAIS être
        // ré-upsertées ici (l'upsert écraserait leur nom par un placeholder). Un node_id
        // fantôme (nœud purgé) ferait échouer la FK : le catch par document de enrichAll gère.
        graph.upsert(new ExtractionResult(topicNodes, edges, Set.copyOf(nodeIds)));
