...
class ConfluenceConnectorTest {
    ...
    @Test
    void resoutLeSpaceIdListeLesPagesEtPagineParCursor() {
        var builder = RestClient.builder().baseUrl("https://conf.test/wiki");
        var server = MockRestServiceServer.bindTo(builder).build();
        var config = new TeamConfig.Confluence("https://conf.test/wiki", List.of("FPSSUITE"));
        var connector = new ConfluenceConnector(config, builder.build());
        ...
        assertThat(docs.get(0).title()).isEqualTo("Page A");
        assertThat(docs.get(0).content()).contains("Contenu A");
        assertThat(docs.get(0).url()).isEqualTo("https://conf.test/wiki/spaces/FPSSUITE/pages/65735/Page+A");
        assertThat(docs.get(0).metadata()).containsEntry("space", "FPSSUITE");
        assertThat(docs.get(1).path()).isEqualTo("65800");
        server.verify();
    }

    @Test
    void arreteLaPaginationDesQuUnePageEstAnterieureASince() {
        var builder = RestClient.builder().baseUrl("https://conf.test/wiki");
        var server = MockRestServiceServer.bindTo(builder).build();
        var config = new TeamConfig.Confluence("https://conf.test/wiki", List.of("FPSSUITE"));
        var connector = new ConfluenceConnector(config, builder.build());
        ...
        // Aucune requête cursor attendue : on s'arrête sur la page "Vieille" (< since).
