#!/usr/bin/env bash
# Crée les issues factices du projet Jira XRAGSAND (voir jira/issues-sandbox.md).
#
# Prérequis : curl, jq. Le projet (clé XRAGSAND par défaut) doit exister et être
# VIERGE : les MRs et pages Confluence référencent XRAGSAND-1..4 littéralement.
#
...
#   JIRA_PROJECT    (optionnel) clé du projet, défaut XRAGSAND
set -euo pipefail

: "${JIRA_BASE_URL:?JIRA_BASE_URL requis}"
: "${JIRA_TOKEN:?JIRA_TOKEN requis}"
PROJECT="${JIRA_PROJECT:-XRAGSAND}"
...
k1=$(create_issue Story "Suivi de commande dans Fake Orders" \
  "Ajouter un suivi d'état des commandes exposé par l'API de Fake Orders et consommé par le front. Implémentation en cours dans la MR « XRAGSAND-1 Suivi de commande » (fake-orders).")
...
k3=$(create_issue Story "Export CSV des factures" \
  "Export CSV mensuel des factures pour la comptabilité. Livré par la MR « XRAGSAND-3 Export CSV des factures » (fake-billing, mergée).")
